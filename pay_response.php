...
try {
  ...
  if ($session->payment_status == 'paid') {
    ...
    if (mysqli_num_rows($booking_res) > 0) {
      ...
      $updateQuery = "UPDATE `booking_order` SET `booking_status`='booked', `trans_id`=?, `trans_status`='TXN_SUCCESS' WHERE `booking_id`=?";
      insert($updateQuery, [$session->payment_intent, $booking_fetch['booking_id']], 'si');
      ...
      <body>
        <div class="container">
          <div class="success-message">
            ...
            <p>Amount Paid: ' . $_SESSION['room']['currency'] . ' ' . ($session->amount_total / 100) . '</p>
          </div>
